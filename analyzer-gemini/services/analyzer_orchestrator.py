...
class AnalyzerOrchestrator:
    ...
    def analyze_article(self, article_id: int):
        ...
        워크플로우:
            1. 기사 조회
            2. Gemini Phase 1: 종목 추천
            3. 추천 종목별로:
               - KIS API: 주가 데이터 조회
               - Gemini Phase 2: 상승 확률 예측
        ...
        try:
            ...
            # 2. Phase 1: Gemini에서 종목 추천
            try:
                recommendations = self.gemini.recommend_stocks(
                    article.title,
                    article.content
                )
    ...
    def _analyze_stock(self, article, recommendation: Dict) -> bool:
        ...
        try:
            ...
            rec_id = self.repos.recommendation_repo.save_recommendation(
                ...
                llm_model=self.gemini.get_model_name(),
                llm_version=self.gemini.get_model_version()
            )
            ...
            # 4. Phase 2: Gemini로 상승 확률 예측
            try:
                prediction = self.gemini.predict_price_increase(
                    article.title,
                    article.content,
                    stock_code,
                    stock_name,
                ...
                logger.error(f"Gemini prediction failed for {stock_code}: {e}")
                return False
    ...
    def analyze_disclosure(self, disclosure_id: int):
        ...
        try:
            ...
            # 5. Gemini로 공시 분석
            try:
                analysis = self.gemini.analyze_disclosure(
                    corp_name,
                    stock_code,
                ...
                logger.error(f"Gemini disclosure analysis failed for {stock_code}: {e}")
                self.repos.log_repo.log_analysis(
