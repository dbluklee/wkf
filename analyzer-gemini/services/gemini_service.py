"""
Google Gemini API 서비스 (BaseLLMService 구현)
"""
...
class GeminiService(BaseLLMService):
    ...
    def recommend_stocks(self, article_title: str, article_content: str) -> List[Dict]:
        ...
        """
        prompt = f"""당신은 한국 증권 시장 전문 애널리스트입니다.
뉴스 기사를 분석하여 투자 가치가 있는 한국 상장 주식을 추천하세요.

다음 뉴스 기사를 분석하여 투자 종목을 추천하세요.
...
- 반드시 JSON 형식으로만 응답"""
        ...
        try:
            logger.info(f"[Gemini] Requesting stock recommendations for: {article_title[:50]}...")
            ...
            logger.info(f"[Gemini] Recommended {len(recommendations)} stocks")
            ...
            logger.error(f"[Gemini] API error in recommend_stocks: {e}")
            raise
...
- 반드시 JSON 형식으로만 응답"""
        ...
        try:
            logger.info(f"[Gemini] Requesting price prediction for {stock_name}({stock_code})")
            ...
            logger.info(f"[Gemini] Predicted {result.get('probability')}% for {stock_name}")
            return result
            ...
            logger.error(f"[Gemini] API error in predict_price_increase: {e}")
            raise
...
- 반드시 JSON 형식으로만 응답"""
        ...
        try:
            logger.info(f"[Gemini] Analyzing disclosure for {corp_name}({stock_code}): {report_name}")
            ...
            logger.info(f"[Gemini] Disclosure analysis: {result.get('probability')}% for {corp_name}")
            return result

        except json.JSONDecodeError as e:
            logger.error(f"[Gemini] Failed to parse disclosure analysis response as JSON: {e}")
            logger.error(f"Response text: {response_text[:500]}")
            raise
        except Exception as e:
            logger.error(f"[Gemini] API error in analyze_disclosure: {e}")
            raise

    def _format_price_data(self, price_history: List[Dict], intraday_prices: List[Dict]) -> str:
        """
        주가 데이터를 Gemini가 읽기 쉬운 포맷으로 변환
