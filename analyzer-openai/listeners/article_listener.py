...
class ArticleListener:
    ...
    def start_listening(self):
        ...
        새 기사가 INSERT되면 trigger가 NOTIFY를 보내고,
        이 함수가 알림을 받아서 callback 함수 호출
        """
        logger.info("Starting PostgreSQL LISTEN on channel 'new_article'")
        ...
        try:
            ...
            logger.info("Now listening for new articles...")
            ...
            while True:
                ...
                while self.connection.notifies:
                    ...
                    logger.info(f"Received notification: new article_id={article_id}")

                    # 콜백 함수 호출
                    try:
                        self.callback(article_id)
                    except Exception as e:
                        logger.error(f"Error processing article {article_id}: {e}")
                        # 에러가 발생해도 리스너는 계속 동작

        except KeyboardInterrupt:
            logger.info("ArticleListener interrupted by user")
            raise
        except Exception as e:
            logger.error(f"ArticleListener error: {e}")
            raise
        finally:
            if self.connection:
    ...
    def stop_listening(self):
        ...
        if self.connection:
            try:
                cursor = self.connection.cursor()
                cursor.execute("UNLISTEN new_article;")
                self.connection.close()
                logger.info("Stopped listening for new articles")
            except Exception as e:
