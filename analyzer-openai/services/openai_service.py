"""
OpenAI API를 사용한 주식 분석 서비스 (BaseLLMService 구현)
"""
...
class OpenAIService(BaseLLMService):
    ...
    def recommend_stocks(self, article_title: str, article_content: str) -> List[Dict]:
        """Phase 1: 종목 추천"""
        system_message = f"""당신은 한국 증권 시장 전문 애널리스트입니다.
...
- 반드시 JSON 형식으로만 응답"""
        ...
        user_message = f"""다음 뉴스 기사를 분석하여 투자 종목을 추천하세요.
...
{article_content}"""
        ...
        try:
            logger.info(f"[OpenAI] Requesting stock recommendations for: {article_title[:50]}...")
            ...
            logger.error(f"[OpenAI] Failed to parse response as JSON: {e}")
            logger.error(f"Response text: {response_text[:500]}")
            return []
        except Exception as e:
            logger.error(f"[OpenAI] API error in recommend_stocks: {e}")
            raise
...
- 반드시 JSON 형식으로만 응답"""
        ...
        user_message = f"""뉴스: {article_title}
...
이 정보를 바탕으로 {stock_name}({stock_code})의 단기 상승 확률을 예측하세요."""
        ...
        try:
            logger.info(f"[OpenAI] Requesting price prediction for {stock_name}({stock_code})")
            ...
            logger.info(f"[OpenAI] Predicted {result.get('probability')}% for {stock_name}")
            return result
            ...
            logger.error(f"[OpenAI] API error in predict_price_increase: {e}")
            raise
...
이 공시가 {corp_name}({stock_code})의 주가에 미치는 영향을 분석하고 매수 여부를 판단하세요."""
        ...
        try:
            logger.info(f"[OpenAI] Analyzing disclosure for {corp_name}({stock_code}): {report_name}")
            ...
            logger.info(f"[OpenAI] Disclosure analysis: {result.get('probability')}% for {corp_name}")
            return result

        except json.JSONDecodeError as e:
            logger.error(f"[OpenAI] Failed to parse disclosure analysis response as JSON: {e}")
            logger.error(f"Response text: {response_text[:500]}")
            raise
        except Exception as e:
            logger.error(f"[OpenAI] API error in analyze_disclosure: {e}")
            raise
